...
%%
clear p1 p2 p3 p4 p5 p6 p7 p8 set SETT
yn=26;hn=22; alln=18;

morning=6;SETT=building.zone(l).neightags;    l=19;
for day=1
...
for j=1: numel(SETT)
    i=SETT(j);
    figure('Name',['ARRs: Cooling Operation for zone ',building.zone(i).tag],'Position', [100 100 1100 1100])
    subplot(4,1,1)
...
%     set(p4,'MarkerSize',12,'Color',[0.5 0.5 0.7])%,'Linestyle',':')
    ...
    ylim([-5 30])
    y=ylabel('($^oC$)');
    set(y,'Interpreter','latex','FontSize',yn)
    box on
...
%     set(p6,'LineWidth',2.5,'Linestyle','--')%'Color','red'
    ...
    h=legend([p1 p2 p5 p7 p8],['$|\epsilon_{{\rm{z}}_{', num2str(i) ,'}}(k)|$'],...
        ...
        'Cooling','Heating');
    set(h,'Interpreter','latex','FontSize',hn,'Location','northeast','Orientation','horizontal')
    set(gca,'FontSize',alln,'fontweight','bold')
